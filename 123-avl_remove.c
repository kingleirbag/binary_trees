...
/**
 * successor - A function that gets the next successor min node in the right subtree
 * @node: tree to check
...
bst_t *bst_remove(bst_t *root, int value)
{
	...
	{
		type_r = remove_type(root);
		if (type_r != 0)
			bst_remove(root->right, type_r);
	}
	else
		return (NULL);
	return (root);
...
/**
 * avl_remove - a function that builds an AVL tree from an array
 * @root: root of the tree
 * @value: node with this value to remove
 * Return: the tree changed
 */
